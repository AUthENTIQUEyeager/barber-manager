...
export default function BossCoiffures() {
  ...
  async function addCoiffure(e) {
    ...
    try {
      ...
      setForm({ nom: '', prix: '', description: '' }); setShowAdd(false);
      toast(`"${c.nom}" ajouté`);
  ...
  return (
    <BossLayout title="Services" subtitle={`${coiffures.length} service(s) configuré(s)`}
      ...
      : coiffures.length === 0 ? (
        <div className="empty-state">
          <div className="empty-state-icon">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><line x1="20" y1="4" x2="8.12" y2="15.88"/></svg>
          </div>
          <div className="empty-state-text">Aucun service configuré.</div>
        ...
        <div className="table-wrapper">
          <table className="data-table">
            <thead>
              <tr><th>Service</th><th>Description</th><th>Prix</th><th style={{textAlign:'right'}}>Action</th></tr>
            </thead>
            <tbody>
              {coiffures.map(c => (
                <tr key={c.id}>
                  <td><span style={{fontWeight:500}}>{c.nom}</span></td>
                  <td style={{color:'var(--text2)'}}>{c.description || '—'}</td>
                  <td><span style={{fontWeight:600,color:'var(--accent)'}}>{fmt(c.prix)}</span></td>
                  <td style={{textAlign:'right'}}>
                    <button className="btn btn-danger btn-icon" onClick={() => deleteCoiffure(c)}>
        ...
        <div className="modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={addCoiffure}>
              <div className="form-group"><label className="form-label">Nom du service</label>
                <input className="form-input" placeholder="Ex: Dégradé américain" value={form.nom} onChange={set('nom')} required/></div>
              <div className="form-group"><label className="form-label">Prix (FCFA)</label>
                <input className="form-input" type="number" placeholder="1000" value={form.prix} onChange={set('prix')} required/></div>
              <div className="form-group" style={{marginBottom:20}}><label className="form-label">Description (optionnel)</label>
                <input className="form-input" placeholder="Courte description" value={form.description} onChange={set('description')}/></div>
