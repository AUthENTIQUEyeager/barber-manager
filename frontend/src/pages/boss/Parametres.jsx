  ...
  return (
    <BossLayout title="Paramètres">
      ...
      <div className="card" style={{textAlign:'center',marginBottom:12}}>
        <div className="stat-label" style={{marginBottom:8}}>Code d'accès coiffeurs</div>
        <div style={{fontSize:28,fontWeight:700,letterSpacing:6,color:'var(--accent)',margin:'8px 0'}}>
          {salon?.salon_code}
        </div>
      ...
      <div className="card">
        <div className="card-header">
          <span className="card-title">Informations du salon</span>
        </div>
      ...
      <div className="card" style={{border:'1px solid #BFDBFE',background:'#EFF6FF'}}>
        <div style={{marginBottom:12}}>
          <div style={{fontSize:14,fontWeight:600,color:'#1E40AF',marginBottom:4}}>Site web pour votre salon</div>
          <p style={{fontSize:13,color:'#3B82F6',lineHeight:1.5}}>
            Obtenez un site professionnel pour que vos clients vous trouvent en ligne.
          </p>
        </div>
        <button className="btn btn-sm" onClick={openWhatsApp}
          style={{background:'#25D366',color:'white',border:'none',width:'auto'}}>
      ...
      <button className="btn btn-secondary" onClick={async () => { await logout(); navigate('/'); }}
        style={{color:'var(--danger)',borderColor:'#FECACA',marginTop:4}}>
