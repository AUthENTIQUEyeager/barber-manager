...
const NavIcon = ({ path, icon, label }) => {
  const location = useLocation();
  const active = location.pathname === path;
  return (
    <Link to={path} className={`bottom-nav-item ${active ? 'active' : ''}`}>
      <span className="bottom-nav-icon" style={{fontSize:19}}>{icon}</span>
      <span className="bottom-nav-label">{label}</span>
    </Link>
  );
};

export default function BossLayout({ title, children, action }) {
  const { salon } = useAuth();
  return (
    <div className="app-layout">
      ...
      <nav className="bottom-nav">
        ...
        <NavIcon path="/boss/historique" icon={
        ...
        <NavIcon path="/boss/coiffures" icon={
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><line x1="20" y1="4" x2="8.12" y2="15.88"/><line x1="14.47" y1="14.48" x2="20" y2="20"/><line x1="8.12" y1="8.12" x2="12" y2="12"/></svg>
        } label="Services" />
        <NavIcon path="/boss/coiffeurs" icon={
        ...
        <NavIcon path="/boss/parametres" icon={
